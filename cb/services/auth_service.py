...
def get_client(
    ...
    from cb.services.session import load_session, save_session
    session = load_session(db_path)

    if not session:
        try:
            profile = get_profile(profile_name, db_path) if profile_name else get_default_profile(db_path)
            
            if profile and hasattr(profile, 'password'):
                raw_token = _build_basic_token(profile.username, profile.password)
                save_session(raw_token, profile.name, profile.server_url, profile.username, db_path)
                session = load_session(db_path)
        except Exception:
            pass
