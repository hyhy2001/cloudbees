...
bee job list
bee ask "how do I run a job"         # RAG-powered help (requires LM endpoint; retrieval is local)
...
Ask how to use `bee` in natural language. **Requires a configured LM endpoint** (`bee.lm.json` or env) — without one, `bee ask` exits with an error. Retrieval (BM25 + vector + graph + rerank) runs locally; the final answer is generated by the LM. Options: `--limit <n>` (context items, default 5), `--json` (machine-readable output).

```bash
bee ask <query> [--limit <n>] [--json]
...
# Recent build history (alias: `bee job history`)
...
A SQLite TTL cache backs GET calls (controllers, jobs, nodes, credentials — TTLs range 5–300 s by resource type; see [Cache & TTL policy](#cache--ttl-policy-corecache)). Writes automatically invalidate related cache entries.
...
Edit / Track / Untrack live inside the `Enter` action menu on the Nodes and Credentials tabs (Jobs uses the multi-select keys `i`/`u` for track/untrack instead), not as standalone keys.
...
| Controllers | `Enter` | View controller info |
| Controllers | `s` | Select the active controller |
| Info | `Ctrl+x` | Clear the local cache |

The Jobs action menu carries: View Log · View Script (pipeline only) · Run · Stop · Edit · Params · Schedule · Email · Delete · Move · Clone, plus Controlled Agents on folder rows. Inside the menu, `1`–`9` pick directly, `↑`/`↓` move, `Enter` runs, `Esc` backs out to the list.
...
| `CB_EMBEDDING_URL` | Explicit full embedding endpoint URL. Overrides the `CB_DATABRICK_URL` + `CB_EMBEDDING_PATH` composition — use when embedding is served from a different host than chat |
