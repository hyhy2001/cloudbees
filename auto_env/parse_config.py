...
def cmd_plan_stale_jobs(cfg, manifest_path):
    """Emit STALE_JOB <folder/job> for every manifest job the current plan no longer
    includes. deploy.csh then checks each one's ACTUAL schedule via `bee job get` and
    only clears a timer that really exists — so a scheduleless manual job is a no-op,
    and this stays correct even if job_name changed between deploys (we don't guess
    from the name)."""
    try:
